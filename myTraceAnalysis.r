...
library(timereg) # TRACE Data
library(survival) # Surv, survreg
library(randomForest) # randomForest
library(randomForestSRC) # rfrsc
...
random.forest = function(obs,delta,xx) # mfunc calls this
{
  nu = length(obs)
...
# Output: obs = observed failure time adjusted for truncation, delta = failure indicator adjusted for truncation
...
externalRegularKM=function(obs,delta,xx, mtype,dtype) # parameters for DoublyRobustL2
{
  n = length(obs)
  nu = n
...
# mtype = type of conditional expectation, dtype = truncation method and level.
BJexternal=function(obs,delta,xx, mtype,dtype, time.point) # parameters for BuckleyJamesL2
{
  n = length(obs)
  nu = n
...
default.Wei <- function(data.train, data.test) {#
  ...
  for(i in 1:nrow(data.test)){
    ...
    surv.prob[i] <- S_t(3, model$scale, predict.test[i])
  }
...
DoublyRobustL2 <- function(data.train, n.tree, tau1, data.test, mtype, dtype){
  ...
  for(j in 1:n.tree){
    ...
    predict.test <- randomForest:::predict.randomForest(tree.one, newdata = data.test)
    predict.train <- randomForest:::predict.randomForest(tree.one, newdata = xx)
...
BuckleyJamesL2 <- function(data.train, n.tree, tau1, data.test, mtype, dtype){
  ...
  for(j in 1:n.tree){
    ...
    # terminal node they fall. 
    predict.test <- randomForest:::predict.randomForest(tree.one, newdata = data.test)
    predict.train <- randomForest:::predict.randomForest(tree.one, newdata = xx.train)
...
run = function(data.trace){  
  tau1 = 3 # we want to predict P(T > t | W) where t = 3
  ...
  get.sam <- rep(c(1:v),sum(delta[fitset]==TRUE)/v) # 1 2 3 ... 10 1 2 3 ... 8 9 10 (78 times)
  
  # if v doesn't divide the sum(delta[fitset]==TRUE), we have remaining data that are not assigned:
  if(length(get.sam)<sum(delta[fitset]==TRUE)){
    remaining <- c(1:(sum(delta[fitset]==TRUE)-length(get.sam)))
    get.sam <- c(get.sam,remaining)
  }
  ...
  # we may have different sizes for groups with delta = 0
  # in this run, we have the following delta.0 distribution:
  # 1 - 90 2 - 76 3 - 89 4 - 101 5 - 91 
  # 6 - 88 7 - 92 8 - 92 9 - 104 10 - 86
  # total - 909
  
  # assigning to v groups
  k <- 1
  l <- 1
  grp.delt <- NULL 
  for(m in 1:nu){
    ...
    if(k == 909) {
      if (l == 780) {
        print("all data assigned to 10 groups")
      }
    }
  }
  ...
  for(k in 1:v){
    ...
    data.train <- data.trace[grp.delt != k, ]
    data.test <- data.trace[grp.delt == k, ]

    # obs - time, delta - status
    obs.train <- data.train[, 6]
    delta.train <- data.train[, 1]
    obs.test <- data.test[, 6]
    delta.test <- data.test[, 1]
    
    # covariates (chf, age, sex, diabetes, vf)
    #xx.train <- data.train[, -c(1,6)]
    xx.test <- data.test[, -c(1,6)]
    #xx.all <- data.trace[, -c(1, 6)]
    
    # test set
    data.test = xx.test
    
    # Setting obs and delta to be first two columns in data.train
    data.train = data.train[, c(6, 1, 2:5, 7)]
    
    n <- length(obs) # ///
    num <- length(obs) # ///
    
    # Number of trees in the forest.
    n.tree <- 1000 # page 379
    
    mtype = "rand.for" 
    dtype = "b10" # truncation method
    obs.test.t = pmin(obs.test, tau1) # tau1 = 3 to predict S0(t|W) where t = 3, page 381
    delta.test.t = delta.test * (obs.test <= tau1) + (obs.test > tau1) # page 376
    gfunckm_res = GfuncKM(obs.test.t,delta.test.t,dtype)
    a0 <- delta.test.t/GfuncKM(obs.test.t,delta.test.t,dtype)[[1]] # page 376 
    a0s = c(a0s, a0)
...
main <- function(){
  ...
  # Removing observations with undefined censoring status.(NOTE: STAT 437 removed status = 7 as well)
  TRACE <- TRACE[TRACE[['status']] != 8, ] 
  # Only including patients surviving past 30 days  
  TRACE = TRACE[TRACE[['time']] * 365 > 30, ]
  
  # negligible variations to time in order to break ties
  obs <- TRACE[['time']] + runif(dim(TRACE)[1], -10^-9, 10^-9) 
  
  delta <- as.numeric(TRACE[['status']] == 9)

  # Remove id and wmi
  data.trace = TRACE[, -c(1,2)] 

  data.trace[['time']] <- obs
  data.trace[['status']] <- delta
  
  result <- run(data.trace)
  return(result)
}
